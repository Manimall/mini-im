...
const Phones = () => {
  ...
  const renderPhone = React.useCallback((phone) => {
  ...
  const renderedPhones = React.useMemo(() => phones.map(renderPhone), [phones, renderPhone]);

  const render = () => {
    if (isLoading && phones.length === 0) return <Loader/>;

    if (error) return <Error/>;

    if (!isLoading && phones.length === 0) return <NoResults/>;

    if (phones.length > 0 && isLoading) return (
      <>
